...
def load_TargetCNN(model,name):
    ...
    pretrained_dict = torch.load("pre-trained model/pretrained_model.pth")
    new_dict = model.state_dict()
    pretrained_dict1 = {k: v for k, v in pretrained_dict.items() if k in new_dict}
    new_dict.update(pretrained_dict1)
    model.load_state_dict(new_dict)
    
    if name=='FinetuneTail':
        namelist = ['predict.weight','predict.bias']
...
if __name__ == '__main__':
    ...
    for i in range(seed_number):
        ...
        np.random.seed(SEED)
        #随机取点
        index1 = np.random.choice(c4.shape[0],target_size)
        ...
        t_xtrain = c6[index1,:,:]
        t_ytrain = Tool_62[index1,:]
        t_xtest = np.delete(c6, index1, axis = 0)
        t_ytest = np.delete(Tool_62, index1, axis = 0)
        
        s_xtrain = torch.Tensor(c4)
        s_ytrain = torch.Tensor(Tool_42)
...
####11111111111111111111111111111111Only target11111111111111111111111111111111111111111####        
        if 1:
            '''
            仅使用目标域数据进行训练
            '''
